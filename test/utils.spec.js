...
const { merge, set, get, handleObjectPath } = require("../lib/utils");
...
describe("lib/utils", () => {
  ...
  describe("merge", () => {
    ...
    it('should replace object with simple key in target', () => {
      ...
      expect(merge(target, src), expected);
    });
    ...
    it('should work on array properties', () => {
      ...
      expect(merge(target, src)).to.deep.equals(expected);
      expect(Array.isArray(merge(target, src).key1)).to.be.true;
      expect(Array.isArray(merge(target, src).key2)).to.be.true;
    });
    ...
    it('should work on array of objects', () => {
      ...
      expect(merged).to.deep.equals(expected);
      expect(Array.isArray(merge(target, src))).to.be.true;
      expect(Array.isArray(merge(target, src)[0].key1)).to.be.true;
